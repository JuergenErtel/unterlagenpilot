...
export function generateFileName(input: FilenameInput): string {
  ...
  if (
    input.propertyRef &&
    ["grundbuchauszug", "expose", "flurkarte_lageplan", "teilungserklaerung", "wohnflaechenberechnung", "grundriss", "ansichten", "skizze", "objektfoto", "kaufvertragsentwurf"].includes(
      type
    )
  ) {
