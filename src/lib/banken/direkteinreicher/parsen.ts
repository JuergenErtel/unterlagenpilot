...
export function parseDirekteinreicher(html: string): Direkteinreicherinfo {
  ...
  for (const b of bloecke(html)) {
    ...
    const personen = ansprechpartnerAusTabelle(b.zeilen);
    if (personen && personen.length > 0) {
      ansprechpartner.push(...personen);
      continue;
    }
    const text = b.zeilen.map((z) => z.filter((x) => x).join(" · ")).filter(Boolean).join("\n");
    if (!text) continue;
    if (istAnschrift(ueberschrift)) anschriften.push(text);
