...
import {
  portalUploadOne,
  portalFinishUpload,
  portalRequestUploadSlot,
  portalProcessStoredUpload,
...
export interface PortalUploadApplicant {
  position: number;
  name: string;
}

/**
 * Upload des Auftraggebers in die Akte seines Auftrags - dieselbe Mechanik
 * wie der Vermittler-Upload (Einzeldateien, Direkt-Upload fuer grosse
 * Dateien), aber ueber die Portal-Actions, die den Auftrag pruefen.
 *
 * Bei mehreren Antragstellern ist KEINE Person vorausgewaehlt: Eine
 * mitgegebene Zuordnung gilt als manuell und wird von der Namenserkennung
 * nie mehr korrigiert (siehe broker-upload-form.tsx).
 */
export function PortalUploadForm({
  ...
  auftragId: string;
  maxMb: number;
  applicants: PortalUploadApplicant[];
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const outcome = await uploadFilesSequentially(files, (fd) => portalUploadOne(auftragId, fd), {
        extraFields: { applicantPosition },
        onProgress: setProgress,
        requestSlot: (name, mime) => portalRequestUploadSlot(auftragId, name, mime),
        processStored: (meta) => portalProcessStoredUpload(auftragId, applicantPosition, meta),
      });
      await portalFinishUpload(auftragId);
      setResult(outcome);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" className="w-full sm:w-auto" disabled={busy || files.length === 0}>
        {busy ? "Wird übertragen …" : files.length > 0 ? `${files.length} Datei(en) hochladen` : "Hochladen"}
      </Button>
      ...
      {result && result.uploaded > 0 ? (
        <p className="flex items-center gap-2 rounded-md bg-success/10 px-3 py-2 text-sm text-success">
          ...
          {result.uploaded} Datei(en) an das Backoffice übergeben. Die Prüfung erfolgt dort.
        </p>
      ) : null}

      {result && result.rejected.length > 0 ? (
        <div className="space-y-1.5 rounded-md bg-warning/10 px-3 py-2 text-sm" role="alert">
          <div className="flex items-center gap-2 font-medium text-[hsl(var(--warning))]">
