...
describe("starteAnfrage", () => {
  it("legt Link und Bogen an und schickt auf den naechsten Schritt", async () => {
    const { ziel } = await starten("ertel", { "finanzierungsart.art": "kauf_bestand" });
  ...
  it("legt nichts an, wenn die IP-Grenze erreicht ist", async () => {
    ...
    const { ergebnis } = await starten("ertel", { "finanzierungsart.art": "kauf_bestand" });
  ...
  it("prueft die IP-Grenze VOR der Datenbankabfrage zum Formular", async () => {
    ...
    checkRateLimit.mockResolvedValue({ ok: false, remaining: 0, retryAfterSec: 3600 });
    await starten("ertel", { "finanzierungsart.art": "kauf_bestand" });
  ...
  it("legt nichts an, wenn das Formular unbekannt oder abgeschaltet ist", async () => {
    ...
    const { ergebnis } = await starten("gibtsnicht", { "finanzierungsart.art": "kauf_bestand" });
    expect(ergebnis?.error).toBeTruthy();
    expect(createAnfrageLink).not.toHaveBeenCalled();
  });

  it("weist einen ungueltigen Wert ab, statt ihn zu speichern", async () => {
    const { ergebnis } = await starten("ertel", { "finanzierungsart.art": "brieftaube" });
  ...
  it("gibt zwei Besuchern getrennte Boegen", async () => {
    ...
    const erster = await starten("ertel", { "finanzierungsart.art": "kauf_bestand" });
    const zweiter = await starten("ertel", { "finanzierungsart.art": "kauf_bestand" });
